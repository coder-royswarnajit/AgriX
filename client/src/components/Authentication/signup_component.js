...
export default class SignUp extends Component {
  ...
  onSignInSubmit(e) {
    ...
    fetch(`${process.env.REACT_APP_API_URL}/farmer/mobilenumverify/${Mobilenum}`, {
      method: "get",
    })
      .then((res) => res.json())
      .then((data) => {
  ...
  onAdharnumSubmit(e) {
    ...
    fetch(`${process.env.REACT_APP_API_URL}/farmer/adhar`, {
      method: "POST",
      crossDomain: true,
      headers: {
    ...
    if (this.state.verified) {
      if (this.state.Password === this.state.Confirmpass) {
        if (
          ...
          fetch(`${process.env.REACT_APP_API_URL}/farmer/farmersignup`, {
            method: "POST",
            crossDomain: true,
            headers: {
              "Content-Type": "application/json",
    ...
    this.setState({ District: e.target.value }, async function () {
      try {
        const response = await fetch(
          `${process.env.REACT_APP_API_URL}/District/${this.state.District}`
        );
  ...
  handleTaluka(e) {
    this.setState({ Taluka: e.target.value }, async function () {
      try {
        const response = await fetch(
          `${process.env.REACT_APP_API_URL}/District/${this.state.District}/${this.state.Taluka}`
        );
        const data = await response.json();
        console.log(data);
